...
impl FileDivider {
    ...
    pub fn upload(filename: &str, piece_size: u64, file_path:String) -> Option<String> {
        ...
        pieces_info_file.write_all(pieces_string.as_bytes()).expect("failed to write pieces info");
        hash_file.write_all(data_hash).expect("failed to write");
        Some(f.file_hash.clone())
    }
}
